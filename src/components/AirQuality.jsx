...
function getAqiLabel(aqi, t) {
  switch (aqi) {
    case 1:
      return { text: t('airQualityAqiGood'), color: 'text-emerald-500 dark:text-emerald-300' };
    case 2:
      return { text: t('airQualityAqiFair'), color: 'text-lime-500 dark:text-lime-300' };
    case 3:
      return { text: t('airQualityAqiModerate'), color: 'text-yellow-600 dark:text-yellow-300' };
    case 4:
      return { text: t('airQualityAqiPoor'), color: 'text-orange-600 dark:text-orange-300' };
    case 5:
      return { text: t('airQualityAqiVeryPoor'), color: 'text-red-600 dark:text-red-300' };
    default:
      return { text: t('airQualityAqiUnknown'), color: 'text-slate-600 dark:text-slate-300' };
  }
}
...
export function AirQuality({ location }) {
  ...
  const fetchAir = useCallback(async () => {
    ...
    try {
      const res = await axios.get(`${OPEN_WEATHER_BASE_URL}/air_pollution`, {
        params: {
          lat: location.lat,
          lon: location.lon,
  ...
  if (error) {
    return <ErrorCard message={error} onRetry={fetchAir} />;
  }

  if (!data) return null;

  const aqi = data.main.aqi;
  const aqiInfo = getAqiLabel(aqi, t);
